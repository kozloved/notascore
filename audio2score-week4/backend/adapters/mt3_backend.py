...
class MT3Backend:
    ...
    def transcribe_notes(self, audio_path: str | Path) -> list[NoteEvent]:
        ...
        if settings["provider"] == "runpod" and timeout == 300:
            # Existing VPS env still has 300 from the /runsync era. Cold start
            # often takes ~5 minutes, so wait 10 minutes instead.
            timeout = 600

...
def start_runpod_warmup() -> dict:
    """Do not queue a dummy RunPod job.

    Max workers is 1. A warmup /run plus the real transcription leaves two jobs
    on one initializing worker and freezes the endpoint. The recording job is
    the only request that should start the worker.
    """

...
def test_polyphonic_transcription_posts_run_not_runsync(tmp_path, monkeypatch):
    """One MT3Backend.transcribe_notes call → one RunPod /run, no warmup."""
    ...
    assert notes
    assert len(calls) == 1
    assert calls[0] == "https://api.runpod.ai/v2/g40wir5ey71e3/run"
...
def test_warmup_shaped_output_is_not_treated_as_midi(tmp_path, monkeypatch):
            ...
            json.dumps(
                {
                    ...
                    "output": {
                        ...
                        "warmup": True,
                        "model": "yourmt3",
                        "timing": {"inference_seconds": 0.0, "total_seconds": 0.0},
                    },
                }
            ).encode()
        )
    ...
    monkeypatch.setattr("adapters.mt3_backend.urllib.request.urlopen", fake_urlopen)
    with pytest.raises(TranscriptionError, match="skipped inference"):
        MT3Backend().transcribe_notes(audio)


def test_warmup_entrypoint_removed():
    import adapters.mt3_backend as mt3

    assert not hasattr(mt3, "start_runpod_warmup")
    assert runpod_async_url("https://api.runpod.ai/v2/abc") == (

...
The adapter queues `/run` and polls `/status` so a 5-minute worker boot does not fail the site job. `/runsync` in the env URL is still accepted. Manual check (same adapter as production jobs):

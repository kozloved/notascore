...
// ECG-like single heartbeat (P–QRS–T) over t in [0, 1].
...
export default function HeartbeatBackground({ bpm = 72 }: { bpm?: number }) {
  ...
  useEffect(() => {
    ...
    let H = 0;
    let lines: number[] = [];
    let colors = { line: "rgba(16,20,28,0.14)", pulse: "#c4a35a" };
    ...
    function resize() {
      ...
      for (let i = 0; i < count; i += 1) {
        lines.push(Math.round(top + i * LINE_GAP));
      }
    ...
    let activeLine = 0;
    let headX = -WIN;
    let phase: "sweeping" | "pausing" = "sweeping";
    let pauseUntil = 0;
    ...
    function frame(now: number) {
      ...
      if (phase === "pausing") {
        ...
        headX += currentBpm * 9 * dt;
        if (headX > W + WIN / 2) {
          phase = "pausing";
          const periodic = (60000 / currentBpm) * 0.35;
          const randomPause = Math.random() * 500;

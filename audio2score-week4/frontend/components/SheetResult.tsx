...
async function svgToPng(svg: SVGElement, scale: number) {
...
export default function SheetResult({
  ...
  const [busy, setBusy] = useState<"musicxml" | "midi" | "pdf" | null>(null);
    ...
    async function renderSheet() {
      ...
      try {
        const res = await fetch(
          `${apiUrl}/jobs/${jobId}/result?format=musicxml`
        );
        ...
        if (!cancelled) {
          ...
          setMessage(
            err instanceof Error
              ? err.message
              : "Could not render the sheet preview"
          );
        }
      }
    }
  ...
  const downloadFromApi = async (format: "midi" | "musicxml", ext: string) => {
    setBusy(format);
    setMessage("");
    try {
      const res = await fetch(
        `${apiUrl}/jobs/${jobId}/result?format=${format}`
      );
      ...
      setMessage(
        err instanceof Error
          ? err.message
          : `Failed to download ${ext.toUpperCase()}`
      );
    } finally {
      setBusy(null);
    }
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      setMessage(err instanceof Error ? err.message : "Failed to generate PDF");
    } finally {
      setBusy(null);
    }
  };
    ...
    <div className="sheet">
      ...
      <div className="formats">
        ...
        <button
          ...
          title={
            previewState !== "ready"
              ? "Preview must finish rendering first"
              : undefined
          }
        >
          {busy === "pdf" ? (
